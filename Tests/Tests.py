...
class Orthanc(unittest.TestCase):
    ...
    def test_incoming_jpeg(self):
        ...
        if IsOrthancVersionAbove(_REMOTE, 1, 9, 0):
            ...
            self.assertLessEqual(42, len(a))  # 42 in 1.9.0, 45 from 1.12.10+
    ...
    # Helper method to use e.g when comparing zip sizes because the zip size may depend on timestamp and other stuffs.
    # We have seen differences of 4 bytes but let's take a 0.1% margin
    # We have also seen 4 bytes difference in size when downloading a transcoded DICOM file probably due to DICOM tags
    # not having exactly the same value/length
    def assertSizesAlmostEqual(self, sizeA, sizeB, tolerance = None):
        if not tolerance:
    ...
    def test_transcode_lossy_compression_ratio(self):
        ...
        if IsOrthancVersionAbove(_REMOTE, 1, 12, 7) and HasExtendedFind(_REMOTE):
            ...
            if HasGdcmPlugin(_REMOTE):
                ...
                self.assertSizesAlmostEqual(size40, size80)
    ...
    def test_download_file_transcode(self):
        if IsOrthancVersionAbove(_REMOTE, 1, 12, 2):
            ...
            if IsOrthancVersionAbove(_REMOTE, 1, 12, 7):
                ...
                if HasGdcmPlugin(_REMOTE):
                    # The plugin SDK doesn't currently allow to pass lossyQuality to plugins
                    self.assertSizesAlmostEqual(len(content40), len(content80))
                else:
    ...
    def test_find_labels(self):
        ...
        if (IsOrthancVersionAbove(_REMOTE, 1, 12, 0) and
            ...
            if not HasMySQLIndexPlugin(_REMOTE) and not HasODBCIndexPlugin(_REMOTE):
                if IsOrthancVersionAbove(_REMOTE, 1, 12, 11): 
    ...
    def test_pagination_and_limit_find_results(self):
        ...
        self.assertEqual(11, len(DoGet(_REMOTE, '/studies')))
    ...
    def test_archive_utf8(self):
        ...
        if IsOrthancVersionAbove(_REMOTE, 1, 12, 12):
            ...
            self.assertEqual(1, len(z.namelist()))
            self.assertIn(u'5Yp0E - BRAINIX/0 - IRM cérébrale, neuro-crâne/401 - MR - sT2W FLAIR/MR000001.dcm', z.namelist())
